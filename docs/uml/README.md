# 🏛️ Architecture et Diagrammes UML - FIANGEP Pharma

Bienvenue dans le dossier de documentation d'architecture et de modélisation de **FIANGEP Pharma**. Ce dossier contient l'ensemble des diagrammes UML décrivant le fonctionnement, la structure et le déploiement de l'application de gestion de pharmacie.
...
| :--- | :--- | :--- | :--- |
| **Cas d'Utilisation** | Comportemental | Modélise les rôles et permissions des acteurs réels de l'application (**Directeur**, **Magasinier** et **Caissier**). | 🔗 [use_case_diagram.md](file:///c:/wamp64/www/pharma-app/docs/uml/use_case_diagram.md) |
| **Séquence** | Interaction | Illustre la dynamique de création d'une facture de vente (déstockage FEFO) et le flux de connexion (Authentification). | 🔗 [sequence_diagram.md](file:///c:/wamp64/www/pharma-app/docs/uml/sequence_diagram.md) |
...
L'une des fonctionnalités phares de **FIANGEP Pharma** est la gestion intelligente du stock par le principe du **FEFO**. Contrairement à un FIFO classique, le système sélectionne en priorité les médicaments dont la **date d'expiration est la plus proche**, quel que soit leur ordre d'arrivée physique dans la pharmacie.
