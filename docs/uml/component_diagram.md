...
graph TD
    ...
    classDef browser fill:#eff6ff,stroke:#2563eb,stroke-width:1.5px,color:#1e3a8a;
...
C'est le point de contact avec l'utilisateur. Elle s'appuie sur le framework **Bootstrap 5** pour garantir une adaptabilité totale (mobile, tablette, desktop) et un support natif du mode sombre. Le code JavaScript (`JS_Cart` au sein de `caissier/facture.php`) assure le calcul local instantané des remises, de la TVA, du sous-total, et gère le catalogue interactif.
...
* **Module Directeur** : Contient les interfaces d'administration lourdes (CRUD des médicaments, des fournisseurs et du personnel caissier, rapports de ventes).
* **API de génération d'impression** : Fait office de micro-service de reporting. Il compile à la volée les reçus au format imprimable à partir de l'ID de vente fourni.

### 3. Helpers et Noyau (`includes/`)
Ce sont les composants partagés transverses :
* **`header.php` / `footer.php`** : Composants de rendu structurel.
* **`functions.php`** : Regroupe les fonctions logiques globales (vérification des permissions par session, génération automatique de codes-barres uniques ou de factures, et le moteur **FEFO**).
...
La base de données MySQL `pharma_db` centralise les tables du système et garantit l'intégrité des relations (par exemple, la contrainte de clé étrangère empêchant de supprimer une catégorie de médicaments si celle-ci est encore associée à un produit en stock).
